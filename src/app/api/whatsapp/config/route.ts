...
export async function POST(request: Request) {
  try {
    ...
    const { phone_number_id, waba_id, access_token, verify_token, pin, webhook_url } = body
    ...
    if (claimed) {
      return NextResponse.json(
        {
          error:
            'This WhatsApp phone number is already linked to another account on this instance. Each phone number can only be connected to one Sultan Dashboard user.',
        },
        { status: 409 }
      )
    }
